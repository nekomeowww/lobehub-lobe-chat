...
export class UserMemoryModel {
  ...
  search = async (params: SearchUserMemoryParams): Promise<UserMemoryResult[]> => {
    ...
    // Apply ordering - use similarity if embedding provided, otherwise lastAccessed
    if (embedding) {
    ...
    // Update access count and timestamp for returned memories
    if (results.length > 0) {
  ...
  searchWithEmbedding = async (
    ...
    // Start building query with similarity
    let query = this.db
      .select({
        accessedCount: userMemories.accessedCount,
        details: userMemories.details,
  ...
  private updateAccessMetrics = async (memoryIds: string[]): Promise<void> => {
    // Update access count and timestamp for the memories
  ...
  private async ensureMemoriesBelongToUser(memoryIds: string[], allowEmpty = false): Promise<void> {
  ...
  private async assertPreferenceOwnership(
    preferenceId: string,
